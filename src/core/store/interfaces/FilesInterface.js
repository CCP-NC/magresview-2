...
import { makeSelector, BaseInterface, getSel, getNMRData, getB0 } from '../utils';
...
class FilesInterface extends BaseInterface {
    ...
    _efgMakeTable(view, multiplicity) {
        ...
        const B0 = getB0(this.state);

        let table = '# EFG Table generated by MagresView 2\n';
        if (this.includeEuler)
            table += `# Euler angles convention: ${conv}\n`;
        if (B0 !== null) {
            table += `# B0 = ${B0} T (for second-order quadrupolar shifts, `;
            table += 'central transition under MAS)\n';
        }
        ...
        // Combined MS+EFG quadrupolar quantities (null for non-quadrupolar
        // sites, or, for dobs, when no chemical shift reference is set)
        const quadOptions = { B0: B0 };
        const msRefs = this.state.ms_references;
        const PQ = getNMRData(view, 'PQ', 'efg', null, quadOptions)[1];
        const qis = getNMRData(view, 'qis', 'efg', null, quadOptions)[1];
        const dobs = this.hasMSData ?
            getNMRData(view, 'dobs', 'efg', msRefs, quadOptions)[1] :
            PQ.map(() => null);
        ...
        view.atoms.forEach((a, i) => {
            ...
            // EFG data
            if (this.includeEFG) {
                // null (non-quadrupolar site / missing reference) prints as
                // an empty field
                const orBlank = (v) => (v === null ? '' : v);
                table_values = table_values.concat([Vzz[i], aniso[i], asymm[i], Q[i],
                                                    orBlank(PQ[i]), orBlank(qis[i]), orBlank(dobs[i])]);
            }    
